...
6. **AI Classification:** Uses Vercel Gateway (OpenAI-compatible). Prompt asks for structured JSON with `relevant`, `fitScore`, `title`, `description`, `location`, `modality`, `englishLevel`, `technologies[]`, `applicationEmail`, `applicationLink`. All fields nullable except `relevant`. Session cache per hash, FIFO queue (max 1 concurrent).
...
- `popup.html` & `popup.js`: AI config & debug settings UI
...
- `lib/settings.js`: Config persistence (`getEnabled`, `getAIConfig`, `getDisplayConfig`)
