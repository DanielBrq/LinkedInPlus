...
// Call OpenAI-compatible gateway with fetch
...
async function callGateway(prompt, config) {
  ...
  const res = await withTimeout(fetch(url, {
    ...
    console.warn(`${LOG_PREFIX} AI fetch failed — check API key & gateway URL: ${err.message}`);
    return null;
  });
  ...
  if (!res.ok) {
    ...
    console.warn(`${LOG_PREFIX} AI gateway ${res.status}: ${errText.slice(0, REASON_MAX_LENGTH)}`);
    return null;
  }
...
export async function classifyWithAI(description, hash, userProfile, config = {}) {
  // Missing profile or key → skip AI
  if (!description || !userProfile || !config.apiKey) {
