import {
  ...
  REGEX_BOILERPLATE, REGEX_JSON_EXTRACT, REGEX_WHITESPACE,
...
2. Not a job offer (news/opinion/ad) -> relevant=false
3. Else score fit 0-100
4. relevant=true only if fitScore>=50
...
description: max 8 words, keywords only, no full sentences.
null if unknown. technologies always array. No invented data.
...
function buildUserPrompt(filters, negativeFilters, description, postLinks = []) {
  ...
  const truncated = cleaned.length > MAX_DESCRIPTION_CHARS
    ...
    : cleaned;
  let prompt = `User profile (what I'm looking for):\n${filters}\n`;
  if (negativeFilters) {
    prompt += `\nThings I want to avoid (negative filters):\n${negativeFilters}\n`;
  }
  prompt += `\nJob description:\n${truncated}\n`;
