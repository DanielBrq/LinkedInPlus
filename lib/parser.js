...
// Click "See more" button if description is truncated
/** @param {Element} [container] */
export function expandDescription(container = document) {
...
/** @param {Element} [targetContainer] @returns {string|null} */
export function extractDescription(targetContainer) {
  const root = targetContainer || document;

  // Match against description selectors
  let element = null;
  ...
  // Fallback: container itself matches a selector
  if (!element && root.matches && DESCRIPTION_SELECTORS.some(s => root.matches(s))) {
    element = root;
  }

  if (!element) return null;

  // Expand truncated content before reading text
  expandDescription(element.parentElement || element);
