import { extractDescription, extractPosterInfo, extractHashtags, extractLinks } from './parser.js';
...
async function processContainerBody(container, context) {
  ...
  try {
    ...
    let result;
    try {
      result = await classifyWithAI(description, hash, aiConfig.filters, aiConfig.negativeFilters, aiConfig, links);
    } catch (e) {
