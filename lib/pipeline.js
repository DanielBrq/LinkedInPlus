...
async function processContainerBody(container, context) {
  ...
  try {
    ...
    // Pre-filter: 2+ months old (normalized to 'Nmo' by parser, works for EN/ES)
    ...
    // AI unavailable → leave visible with orange border as diagnostic
    if (result.reason === AI_UNAVAILABLE_REASON) {
      console.warn(LOG_PREFIX, MSG_AI_UNAVAILABLE);
      if (post) post.style.outline = '2px solid #ff9800';
      return;
    }
    ...
    // Hard reject: description contains negative filter term → force irrelevant
    if (result.relevant && aiConfig.negativeFilters) {
      const rawNegTerms = aiConfig.negativeFilters.split(',').map(t => t.trim().toLowerCase()).filter(Boolean);
      const negTerms = rawNegTerms.flatMap(t => t.split(/\s+\bor\b\s+/i)).map(s => s.trim()).filter(Boolean);
      const descLower = description.toLowerCase();
      const NEGATION_RE = /(?:^|(?<=\s))(?:no|sin|without|not|no es|no son)\s+/i;
      if (negTerms.some(term => {
        const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const termRe = new RegExp(`${escaped}`, 'gi');
        for (const m of descLower.matchAll(termRe)) {
          const before = descLower.slice(0, m.index);
