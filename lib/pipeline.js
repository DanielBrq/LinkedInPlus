...
function applyNegativeFilter(result, description, negativeFilters) {
  ...
  if (negTerms.some(term => {
    ...
    for (const m of descLower.matchAll(termRe)) {
      if (!NEGATION_RE.test(descLower.slice(0, m.index))) return true;
    }
    return false;
  })) {
    result.relevant = false;
...
async function processContainerBody(container, context) {
  ...
  try {
    ...
    applyNegativeFilter(result, description, aiConfig.negativeFilters);
