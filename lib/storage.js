...
export async function clearSavedJobs() {
  ...
  const keepHashes = new Set(jobs.filter(j => j.locked).map(j => j._hash));
  await storageSet(INDEX_KEY, Array.from(keepHashes));
}
